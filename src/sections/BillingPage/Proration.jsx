...
const Proration = () => {
  ...
  const [isPricingToggled, setIsPricingToggled] = useState(false);
  ...
  const isAutoModeActiveRef = useRef(true);
  const isPricingAutoModeActiveRef = useRef(true);

  // ========== ADDED: Track and cleanup intervals ==========
  ...
  const resetAllStates = () => {
    ...
    setIsPricingToggled(false);
    setIsPricingAutoMode(true);
    setSelectedPricingModel("Usage Based");
  };
  ...
  const handleRestartAll = () => {
    ...
    isPricingAutoModeActiveRef.current = true;
    
    // Restart intervals
    const startDelay = setTimeout(() => {
      if (componentMountedRef.current) {
        startProportionInterval();
        startPricingInterval();
      }
    }, 100);
  ...
  const handleStopAll = () => {
    console.log('Stopping all propogation animations...');
    clearAllAnimations();
    isAutoModeActiveRef.current = false;
    isPricingAutoModeActiveRef.current = false;
    
    // Reset states to initial values
    resetAllStates();
  };

  // ========== MODIFIED: Proportion card auto toggle ==========
  const startProportionInterval = () => {
    if (!componentMountedRef.current || !isAutoModeActiveRef.current) return;
    
    clearAllAnimations(); // Clear any existing intervals
    
    const interval = setInterval(() => {
      if (!componentMountedRef.current || !isAutoModeActiveRef.current) {
  ...
  // ========== MODIFIED: Pricing card auto toggle ==========
  const startPricingInterval = () => {
    if (!componentMountedRef.current || !isPricingAutoModeActiveRef.current) return;
    
    const interval = setInterval(() => {
      if (!componentMountedRef.current || !isPricingAutoModeActiveRef.current) {
        clearInterval(interval);
        return;
      }
  ...
  const days = Array.from({ length: 30 }, (_, i) => {
    const day = i + 1;
    const opacityMap = {
      1: 0.77, 2: 0.77, 3: 0.77, 4: 0.77, 5: 0.49,
      6: 0.77, 7: 0.77, 8: 0.52, 9: 0.77, 10: 0.77,
      11: 0.36, 12: 0.46, 13: 0.77, 14: 0.77, 15: 0.29,
      16: 0.4, 17: 0.77, 18: 0.77, 19: 0.1, 20: 0.77,
      21: 0.77, 22: 0.77, 23: 0.77, 24: 0.77, 25: 0.77,
      26: 0.77, 27: 0.77, 28: 0.77, 29: 0.77, 30: 0.77,
    };
  ...
  // ========== MODIFIED: useEffect for Proportion card ==========
  useEffect(() => {
    if (isAutoMode) {
      isAutoModeActiveRef.current = true;
      startProportionInterval();
    ...
    return () => {
      // Cleanup handled by clearAllAnimations
    };
  }, [isAutoMode]);

  // ========== MODIFIED: useEffect for Pricing card ==========
  useEffect(() => {
  ...
  // ========== MODIFIED: Component mount/unmount ==========
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        console.log('Page hidden, pausing propogation animations');
        clearAllAnimations();
      } else {
        console.log('Page visible, restarting propogation animations');
        if (isAutoModeActiveRef.current) {
          startProportionInterval();
        }
        if (isPricingAutoModeActiveRef.current) {
          startPricingInterval();
        }
      }
    };
    ...
    // Cleanup
    return () => {
      console.log('Cleaning up propogation component');
  ...
  const DebugControls = () => {
    ...
    return (
      <div 
        className="fixed bottom-20 right-4 z-50 flex flex-col gap-2 opacity-70 transition-opacity hover:opacity-100"
        style={{ 
      ...
      >
        ...
        <div className="px-2 py-1 text-xs font-mono bg-gray-100 rounded text-center">
          Prop: {isToggled ? 'ON' : 'OFF'}
          <br />
          Price: {isPricingToggled ? 'Volume' : 'Fixed'}
        </div>
      </div>
    );
  };
          ...
          <div className="relative w-full md:w-72 bg-white rounded-3xl shadow-[0px_40px_80px_-20px_rgba(7,_63,_158,_0.2)] p-6 overflow-hidden">
            ...
            <div className="relative flex justify-between items-start mb-6">
              <div className="relative text-lg font-bold ">
                <h2
                  className={`text-[#1D293D] absolute left-1 -top-[1px] transition-all duration-300
                    ${
                      isToggled
              ...
              <div className="p-5">
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <div
                      className={`w-4 h-4 border rounded-full ${selectedPricingModel === "Free Tiers" ? "bg-[#073F9E] border-[#073F9E]" : "border-[#CAD5E2]"}`}
                    >
                  ...
                  >
                    ...
                    <div
                      className={`w-4 h-4 border rounded-full ${selectedPricingModel === "Hybrid Pricing" ? "bg-[#073F9E] border-[#073F9E]" : "border-[#CAD5E2]"}`}
                    >
                  ...
                  >
                    ...
                    <div
                      className={`w-4 h-4 border rounded-full ${selectedPricingModel === "Usage Based" ? "bg-[#073F9E] border-[#073F9E]" : "border-[#CAD5E2]"}`}
                    >
                  ...
                  >
                    ...
                    <div
                      className={`w-4 h-4 border rounded-full ${selectedPricingModel === "Per unit Pricing" ? "bg-[#073F9E] border-[#073F9E]" : "border-[#CAD5E2]"}`}
                    >
                  ...
                  <div
                    className="flex justify-between items-center py-1.5 px-3 rounded-lg hover:bg-gray-50 cursor-pointer"
                    onClick={() =>
                      handlePricingModelClick("Automatic Proportion")
                    }
                  >
                    <span className="text-sm font-normal text-[#45556C]">
                    ...
                    <div
                      className={`w-4 h-4 border rounded-full ${selectedPricingModel === "Automatic Proportion" ? "bg-[#073F9E] border-[#073F9E]" : "border-[#CAD5E2]"}`}
                    >
